...
build_app(){
	...
	if [ -d "rocksdb_5.14.3" ]; then
		log_normal "[Build App - rocksdb 5.14.3]"
		cd rocksdb_5.14.3
		make db_bench -j 4
		ret=$?
		cd ..
		if [ $ret = 0 ]; then
			log_normal "[Build App - rocksdb_5.14.3].. Done"
		else
			log_error "[Build App - rocksdb_5.14.3].. Error"
		fi
	fi
...
clean(){
	...
	cd rocksdb_5.14.3 && make clean && cd ..
	cd ..
	rm -rf bin
